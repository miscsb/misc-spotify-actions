...
module Misc.SortSerialize where
...
data Serialize a => MergeState a = MergeComplete [a] | MergeIncomplete ([a], [a], [a])
    deriving Show

comparisonToMergeState :: Serialize a => Ordering -> State (MergeState a) (Either (a, a) [a])
...
stepMerge :: Serialize a => MergeState a -> Ordering -> MergeState a
...
data Serialize a => MergeLayerState a = MergeLayerComplete [[a]] | MergeLayerIncomplete ([[a]], MergeState a, [[a]])
    deriving Show

comparisonToMergeLayerState :: Serialize a => Ordering -> State (MergeLayerState a) (Either (a, a) [[a]])
comparisonToMergeLayerState ord = state (`stepMergeLayer` ord)

stepMergeLayer :: Serialize a => MergeLayerState a -> Ordering -> (Either (a, a) [[a]], MergeLayerState a)
...
data Serialize a => MergeSortState a = MergeSortComplete [a] | MergeSortIncomplete (MergeLayerState a)
    deriving Show

comparisonToMergeSortState :: Serialize a => Ordering -> State (MergeSortState a) (Either (a, a) [a])
comparisonToMergeSortState ord = state (`stepMergeSort` ord)

initialMergeSortState :: Serialize a => [a] -> (Either (a, a) [a], MergeSortState a)
...
stepMergeSort :: Serialize a => MergeSortState a -> Ordering -> (Either (a, a) [a], MergeSortState a)
...
currentOrdering :: Serialize a => MergeSortState a -> [[a]]
...
currentOrderingRanks :: Serialize a => MergeSortState a -> [(Int, [a])]
...
estimateComparisonsLeftMerge :: Serialize a => MergeState a -> Int
...
recoverLengthBlockSize :: Serialize a => [[a]] -> [[a]] -> MergeState a -> (Int, Int)
...
estimateComparisonsLeft :: Serialize a => MergeSortState a -> Int
